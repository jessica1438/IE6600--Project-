...
source("www/functions/Functions.R")

#runApp("C:/Users/gene9/Desktop/IE6600-final-project/shinyApp")

## Insert Data
Morticd_final <- read_csv("www/Morticd_final.csv")
Mortality_rate <- read_csv("www/Mortality_rate.csv")%>%rename(mortality=mortality_rate)
Morticd_data <- read_csv("www/Morticd_final.csv")
Morticd_final <- gather(Morticd_final, Age_group, mortality, `Total`,`Age00_01`, `Age01_04`, `Age05_14`,`Age15_24`, `Age25_34`, `Age35_44`,`Age45_54`, `Age55_64`, `Age65above`,`Unspecified_Age`)

Cause_List <- as.character(unique(Morticd_final$Cause))
Country_List <- as.character(unique(Morticd_final$Country))
Year_List <- as.character(unique(Morticd_final$Year))
Sex_List <- as.character(unique(Morticd_final$Sex))
Age_List <- as.character(unique(Morticd_final$Age_group))
...
#https://www.rdocumentation.org/packages/rintrojs/versions/0.3.2/topics/introBox
